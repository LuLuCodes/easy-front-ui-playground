...
import type { Versions } from '../store'
...
export const genJsdelivrLink = (pkg: string, version: string | undefined, path: string) => {
  version = version ? `@${version}` : ''
...
export const genVueLink = (version: string) => {
  ...
  const runtimeDom = genUnpkgLink('@vue/runtime-dom', version, '/dist/runtime-dom.esm-browser.js')
  return {
    compilerSfc,
    runtimeDom,
  }
...
export const genImportMap = ({ vue, elementPlus }: Partial<Versions> = {}, nightly: boolean) => {
